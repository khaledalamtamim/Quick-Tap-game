...
const themes = {
    neon: {
        ...
        ground: "#000",
        grid: "#f8f2fb",
        sunColors: ["#ff0055", "#edda8c"],
        textMain: "rgb(164, 193, 193)",
        textShadow: "#ffffff",
        glow: true,
        scanlines: true,
        particleColor: "#0ff"
...
function toggleTheme() {
    ...
    if (activeTheme === 'neon') {
        ...
        uiLayer.style.color = "rgb(166, 179, 179)";
...
function draw() {
    ...
    if (state.current == state.getReady) {
        ...
        ctx.fillText("Quick Tap", canvas.width / 2, 200);
        ctx.shadowBlur = 0;
        if (!colors.glow) ctx.strokeText("Quick Tap", canvas.width / 2, 200);
